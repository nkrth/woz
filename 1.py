...
class ChatBotApp:
    ...
    def build_chat_frame(self):
        ...
        self.chat_log = tk.Text(self.chat_frame, wrap="word", state="disabled", bg="white", fg="black", font=("Arial", 10))
    ...
    def build_response_frame(self):
        self.response_frame = tk.LabelFrame(self.root, text="RESPONSES", bg="white", fg="black", font=("Arial", 9, "bold"))
        self.response_frame.place(x=340, y=20, width=200, height=150)

        responses = ["It’s sunny today", "Let me check that", "→ Send message"]
        for i, text in enumerate(responses):
            b = tk.Button(self.response_frame, text=text, width=22, anchor="w", relief="groove",
                          command=lambda t=text: self.append_chat("Bot", t))
            b.pack(pady=3)

    def build_config_frame(self):
        self.config_frame = tk.LabelFrame(self.root, text="CONFIGURATION", bg="white", fg="black", font=("Arial", 9, "bold"))
        self.config_frame.place(x=340, y=180, width=200, height=140)

        self.mic_var = tk.BooleanVar(value=True)
        mic_toggle = ttk.Checkbutton(self.config_frame, text="MIC ON", variable=self.mic_var)
        mic_toggle.pack(anchor="w", padx=10, pady=5)

        tk.Label(self.config_frame, text="LOGGING: Active", bg="white", font=("Arial", 9)).pack(anchor="w", padx=10)
        tk.Label(self.config_frame, text="DELAY: 1.5s", bg="white", font=("Arial", 9)).pack(anchor="w", padx=10)

    def build_input_frame(self):
        self.input_frame = tk.LabelFrame(self.root, text="USER INPUT", bg="white", fg="black", font=("Arial", 9, "bold"))
        self.input_frame.place(x=20, y=340, width=520, height=120)

        tk.Label(self.input_frame, text="What’s the weather?", bg="white").pack(anchor="w", padx=10, pady=(5, 0))

        self.input_entry = tk.Entry(self.input_frame, width=60)
        self.input_entry.pack(side="left", padx=10, pady=10)

        send_button = tk.Button(self.input_frame, text="SEND", width=10, command=self.send_user_input)
    ...
    def build_analytics_frame(self):
        self.analytics_frame = tk.LabelFrame(self.root, text="ANALYTICS", bg="white", fg="black", font=("Arial", 9, "bold"))
        self.analytics_frame.place(x=560, y=340, width=220, height=120)

        self.canvas = tk.Canvas(self.analytics_frame, bg="#f9fafb", height=80, width=200)
